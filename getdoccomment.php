...
$commentlist=array();
	$result = mysqli_query($sql->con, "SELECT `user_comment`.*,`user`.`user_name` FROM `user_comment`,`user` where `user_comment`.`doc_id`='$docid' and `user_comment`.`user_account`=`user`.`user_account` ORDER BY `user_comment`.`creat_time` DESC  limit 10");
